...
namespace App\Jobs\Research\WhatsAppConversations;
...
class ResearchWhatsAppConversationsJob implements ShouldQueue
{
    ...
    {
        $this->logInfo('Starting ResearchWhatsAppConversationsJob.', ['attempt' => $this->attempts()]);
        ...
        $researchRun = resolve(WhatsAppConversationsResearchService::class)->research(
            $researchRun, $this->logInfo(...), $this->logError(...),
        );

        $this->logInfo('Finished execution.', [
            'status' => $researchRun->status,
            'conversations' => count($researchRun->knowledge_source_ids),
        ]);
    }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        resolve(ResearchRunService::class)->fail(
            $this->researchRunId, 'No se pudo completar el análisis de las conversaciones de WhatsApp.',
        );
    }
    ...
    private function logInfo(string $message, array $context = []): void
    {
        ...
        Log::channel('ResearchWhatsAppConversationsJobInfo')->info("[{$this->logUuid}] | {$message}", $context);
    }
    ...
    {
        $context = ['researchRunId' => $this->researchRunId, ...$context];
        foreach (['ResearchWhatsAppConversationsJobInfo', 'ResearchWhatsAppConversationsJobErrors'] as $channel) {
