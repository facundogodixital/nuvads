...
namespace App\Jobs\Research\GoogleReviews;
...
use App\Services\GoogleReviewsResearchService;


class ResearchGoogleReviewsJob implements ShouldQueue
{
    ...
    {
        $this->logInfo('Starting ResearchGoogleReviewsJob.', ['attempt' => $this->attempts()]);
        ...
        $researchRun = resolve(GoogleReviewsResearchService::class)->research(
            $researchRun, $this->logInfo(...), $this->logError(...),
        );

        $this->logInfo('Finished execution.', [
            'status' => $researchRun->status,
            'reviews' => count($researchRun->knowledge_source_ids),
        ]);
    }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        resolve(ResearchRunService::class)->fail(
            $this->researchRunId, 'No se pudo completar el análisis de las reseñas de Google.',
        );
    }
    ...
    private function logInfo(string $message, array $context = []): void
    {
        ...
        Log::channel('ResearchGoogleReviewsJobInfo')->info("[{$this->logUuid}] | {$message}", $context);
    }
    ...
    {
        $context = ['researchRunId' => $this->researchRunId, ...$context];
        foreach (['ResearchGoogleReviewsJobInfo', 'ResearchGoogleReviewsJobErrors'] as $channel) {
