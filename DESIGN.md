...
sobre papel oscuro (`:root[data-theme='dark']` en `resources/css/variables.css`). El login
arranca en dark por decisión de producto y ofrece un switch en el cabezal; el resto de la app
heredará la preferencia del sistema.
...
- Módulo de 72px como convención invisible de anchos estructurales (columna 432, bloque 864).
...
- **Azul Suave** (`--accent-soft`, #e9effc): fondo tenue de apoyo del acento.
...
- **Papel Seleccionado** (`--surface-selected`, #e8edf4): estados seleccionados y botones deshabilitados.
- **Hairline** (`--border`, #d7dde6): todas las líneas de 1px del manual.
...
- **Body Small** (400, 0.875rem, 1.5): notas al pie, mensajes de error, link de acceso alternativo.
- **Caption** (400, 0.75rem, 1.5): ayudas de campos en tinta atenuada.
- **Label / Spec** (500, 0.6875rem, tracking 0.14em, MAYÚSCULAS): la etiqueta specimen `.spec-label`; folios de cabezal y pie, labels de formularios, hex de las muestras. Siempre en tinta atenuada.
...
mide 6 módulos exactos (432px, `max-w-[432px]`) y los contenidos de cabezal y pie se alinean
al bloque de 12 módulos (864px, `max-w-[864px]`), como los márgenes de un manual.

La página es una columna vertical de altura completa (`min-h-dvh`): regla de marca de 3px
arriba, cabezal hairline, contenido centrado, pie hairline. Ritmo interno con la escala de
Tailwind (6, 12, 20, 24, 32px; secciones a 40–56px).
...
Geometría de imprenta: rectángulos con esquinas apenas suavizadas (4px, `rounded-sm`) en
botones, inputs, muestras y avisos. Ningún radio mayor, ninguna píldora, ningún círculo
decorativo. Las líneas — la regla de marca de 3px con degradé y los hairlines de 1px — son
la forma recurrente del mundo.

## Components

### Buttons
- **Shape:** esquinas apenas suavizadas (4px).
...
### Cards / Containers
- **Corner Style:** 4px cuando el contenedor lleva borde (aviso de error).
- **Background:** el papel de la página; los contenedores no se elevan.
- **Shadow Strategy:** ninguna (ver Elevation & Depth).
- **Border:** hairline 1px; el aviso de error usa borde y texto en `--danger`.
- **Internal Padding:** 12–16px.

### Inputs / Fields
- **Style:** hairline 1px sobre papel elevado, 4px de radio, padding 10px 14px; label specimen arriba, ayuda en caption de tinta atenuada abajo.
...
### Navigation
- Cabezal y pie como folios de manual: hairline de 1px y etiquetas specimen en los extremos
  del bloque de 864px. Sin barra de navegación tradicional en el build actual.

### Switch de Tema
Botón de 32×32px en el cabezal, hairline 1px y radio 4px, con ícono SVG de trazo 1.5
(sol en dark, luna en light) en tinta atenuada; hover a acento. Alterna `data-theme` en
`<html>`. El login arranca en dark; la app heredará la preferencia del sistema.
...
ease-out. Al abrir, el foco pasa al primer campo.

## Do's and Don'ts

### Do:
- **Do** registrar anchos estructurales al módulo de 72px (columna 432, bloque 864).
...
- **Do** mantener el acento en ≤ un elemento primario por vista: el azul de acción es la primera regla de la página.
...
- **Don't** introducir radios mayores a 4px, píldoras ni tarjetas flotantes centradas sobre fondo gris.
